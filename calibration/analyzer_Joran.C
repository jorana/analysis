...
void analyzer::fit_spectrum(int ichannel, double *fit_range){
    ...
    fit_range[0] -= 100; //Temporary fix by Joran (jorana@nikhef.nl)
    fit_range[1] += 300; //Temporary fix by Joran (jorana@nikhef.nl)
    //
    ...
    // Loading the file
    string ch0ch1_file = "ch0ch1.root";
   
    TFile *f_ch0ch1 = new TFile(ch0ch1_file.c_str(),"READONLY");
    TH1* h_ch0ch1  = (TH1*)f_ch0ch1->Get("ch0ch1");  
    ...
    ch0ch1_frac.push_back(new RooRealVar("ch0ch1","ch0ch1",0.01,0.0,1.0));
    // 
    // 
    //=====================================================================================================================
    //
    ...
    if(PLOT_ON_SCREEN){
		...
		RooPlot* frame2 = E.frame(Title("Residual Distribution")) ;
		RooPlot* frame3 = E.frame(Title("Pull Distribution")) ;
		
		// frame2->SetTitle("");
	  	frame2->addPlotable(hresid,"P") ;
	  	frame2->GetYaxis()->SetTitle("Residual (events)");
	  	frame2->GetXaxis()->SetRangeUser(fit_range[0],fit_range[1]);
	  	
	  	// frame3->SetTitle("");  	
	  	frame3->addPlotable(hpull,"P") ;
	  	frame3->GetYaxis()->SetTitle("Pull");	  	
	  	frame3->GetXaxis()->SetRangeUser(fit_range[0],fit_range[1]);
		...
		char plotpath[128];
		sprintf(plotpath, "/user/jorana/ana_plots/2ch_BG_%d_pk_%i_E_%i_%i.png", ichannel,jcounter,int (fit_range[0]), int (fit_range[1])); //joranchange
		
        c1->Divide(1,3) ;
		c1->cd(1) ; gPad->SetLogy();Eframe->SetMaximum(1.5*maxval); Eframe->SetMinimum(1); gPad->SetBottomMargin(0.15) ; Eframe->GetYaxis()->SetTitleOffset(1.1) ; Eframe->Draw() ;
		c1->cd(2) ; gPad->SetBottomMargin(0.15) ; frame2->GetYaxis()->SetTitleOffset(1.1) ; frame2->Draw() ;
		c1->cd(3) ; gPad->SetBottomMargin(0.15) ; frame3->GetYaxis()->SetTitleOffset(1.1) ; frame3->Draw() ;
...
void analyzer::processFitData(RooRealVar N, RooRealVar f, RooRealVar E, RooRealVar sig, int ichannel, int ipeak){
    ...
    addTreeEntry(E1,R1,dR1,res,ichannel,ipeak, fractry1); //Joranadded ',fractry1'
}

/*----------------------------------------------------------------------------------------------------*/
void analyzer::addTreeEntry(Double_t E, Double_t R, Double_t dR, Double_t res, Int_t ich, Int_t ipk, Double_t fractry){
    //
...
void analyzer::fit_spectrum_simple(int ichannel){
    ...
    for (int ipeak=0; ipeak<MAX_PEAKS; ipeak++){
        if(source_energy[id][ipeak] >0){
            ...
            addTreeEntry(_t_energy,_t_rate,1.0,_t_res,ichannel,ipeak, _t_fractry);
            //            c1->Update();
